...
from solvers.newmark_solver import NewmarkExplicitGPU, NewmarkImplicitForceGPU
...
def setup_module():
    ...
    F = np.zeros((2, 13))
    F[1, :] = 10
    M = sparse.csc_matrix(np.array(M), dtype=np.float64)
    K = sparse.csc_matrix(np.array(K), dtype=np.float64)
    C = sparse.csc_matrix(np.array(C), dtype=np.float64)
    F = sparse.csc_matrix(np.array(F), dtype=np.float64)
...
def test_newmark_sparse(setup_module, preconditioner, newmark):
    ...
    res = newmark(Force(), State(), linear_solver=CGSolverGPU(), preconditioner=prec)
...
def test_newmark_sparse_output_int(setup_module, preconditioner, newmark):
    ...
    res = newmark(Force(), State(output_interval=10), linear_solver=CGSolverGPU(), preconditioner=prec)
    res.initialise(number_eq, time)
    res.calculate(M, C, K, F, 0, n_steps)
    # check solution
    np.testing.assert_array_almost_equal(
...
def test_newmark_sparse_output_int_staged(setup_module, preconditioner, newmark):
    ...
    # run stages
    res = newmark(Force(), State(output_interval=4), linear_solver=CGSolverGPU(), preconditioner=prec)
    res.initialise(number_eq, time)
    # run first stage
...
def test_newmark_dense(setup_module, newmark):
    ...
    res = newmark(Force(), State(), linear_solver=CGSolverGPU(), preconditioner=None)
    res.initialise(number_eq, time)
    res.calculate(M, C, K, F, 0, n_steps)
    # check solution
    np.testing.assert_array_almost_equal(
...
def test_newmark_two_stages(setup_module, preconditioner, newmark):
    ...
    res.calculate(M.copy(), C, K.copy(), F, turning_idxs[0], turning_idxs[1])
    # run second stage
    res.state.update_initial_conditions(turning_idxs[1])
    res.calculate(M.copy(), C, K.copy(), F, turning_idxs[1], len(time) - 1)
